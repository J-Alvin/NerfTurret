...
class CameraTracker:
    ...
    def __init__(self, camera_width = 1920, camera_height = 1080, camera_dead_zone = 0.02):
        self.WIDTH = camera_width
        self.HEIGHT = camera_height
        self.DEAD_ZONE = self.WIDTH * camera_dead_zone
        self.tracked_obj = TrackedObject.TrackedObject()
    ...
    def frame_calc( self, frame, face_cascade):
        ...
        if len(boxes) > 0:
            new_boxes = [boxes[0]]
            boxes = new_boxes
            self.tracked_obj.update(frame, boxes[0])
        else:
            self.tracked_obj.update(frame, None)
        ...
        if(len(boxes) > 0):
            ...
            new_y_offset -= self.HEIGHT / 2

        if ( abs(new_y_offset) > self.DEAD_ZONE):
            if(new_y_offset < 0 ):
                y_offset = -1
            else:
                y_offset = 1
